			       ...
			       void *data, int datalen)
{
	...
	uint32_t ssize = chan->cset->ssize;
	uint32_t ichan = chan->cset->n_chan - 1;
	uint32_t chunksize = ichan * ssize;
	int i;

	while (datalen >= chunksize) {
		for (i = 0; i < 2; i++, ptr++) {
			put_unaligned_le32((*value * gain) + offset, ptr);
			//(*value)++;
		}
		for (i = 2; i < 4; i++, ptr++) {
			put_unaligned_le32(i, ptr);
		}
		for (i = 4; i < 6; i++, ptr++) {
			put_unaligned_le32(*value % 100, ptr);
			//(*value)++;
		}
