...
#define MAX110X0_NICHANNELS 		64  // CHAN_PER_SPI * SPI_DEVICES
...
/* buffer is 10 samples (1980 bytes), and nw that is not transmitted */
#define SAMPLES_PER_BUFFER 10
#define BUFFERS_PER_MICOSI 4 /* 8kB of RAM */

/* sample is 198 bytes (64 * 3 + 6) */
struct adc_sample {
	uint16_t sequence;  /* generated by micosi */
	uint8_t adc_sequence[2];
	uint16_t timestamp; /* clock ticks divided by 1024, or ffff */
	uint8_t data[64][3];
};

struct adc_buffer {
	int nw; /* 0..10; */
	struct adc_sample sample[SAMPLES_PER_BUFFER];
};
// ---------------------------------------
...
/*
	Complete is called in a context which can't sleep.
	The status of the msg xfer is in the msg.status field.
	Until complete callback is returned, no other msg will be 
	processed on the same spi_bus, so there is no need for buffering
*/

int16_t last_seqn = 0;
static void max110x0i_complete(void *arg) {
	...
	struct spi_context *cxt = arg;
	unsigned long flags;
	struct timespec ts;
	struct adc_sample *ibuf = (struct adc_sample *) cxt->xfer.rx_buf;
	uint8_t *burst;
	int16_t seqn;
	int32_t *buf, i, j, tmp,
		nsamples = chan->current_ctrl->nsamples,
		min_space = MAX110X0_NICHANNELS * SAMPLES_PER_BUFFER,
		avail_space = nsamples - cxt->curr_sample;

	// un-arm if buffer is full
	if (unlikely(!block)) {
	...
	if (unlikely(avail_space < min_space)) {
		chan->current_ctrl->zio_alarms |= ZIO_ALARM_LOST_TRIGGER;
	...
	seqn = ibuf[0].sequence;
	if (unlikely((int16_t)(seqn - last_seqn) != 10)) {
		//printk("%05d -> %05d\n", last_seqn, ibuf[0].sequence);
		
		chan->current_ctrl->zio_alarms |= ZIO_ALARM_LOST_TRIGGER;
		getnstimeofday(&ts);
		printk("%lld.%.9ld: lost %d samples\n", (long long)ts.tv_sec, 
			ts.tv_nsec, seqn - last_seqn);
	}
	last_seqn = ibuf[0].sequence;

	buf = block->data;
	buf += cxt->curr_sample;
	// fill zio block with micosi data
	for (i=0; i<SAMPLES_PER_BUFFER; ++i, buf += MAX110X0_NICHANNELS) { 
		burst = (uint8_t *) ibuf[i].data;
		//memcpy(buf, burst, sizeof(struct adc_sample));
		for (j=0; j<MAX110X0_NICHANNELS; ++j, burst+=3) {
			tmp = get_unaligned_be32(burst);
			buf[j] = tmp >> 8;
		}
	}
	//printk("%d -> %d + %d \n", nsamples, cxt->curr_sample, min_space);
	cxt->curr_sample += min_space;
	if (unlikely((nsamples - cxt->curr_sample) <= 0)) {
		buf = block->data;
		cxt->curr_sample = 0;
...
static irqreturn_t max110x0i_gpio_irq(int irq, void *arg)
{
	...
	if (unlikely(gpio_get_value(110))) {
		// FIXME: spurious irq
		//printk("spurious irq\n");
		return ret;
	}
	...
	if (unlikely(spi_async_locked(cxt->spi, &(cxt->msg)))) {
		/* without a spi_async there will be no other irqs */
		printk("error in spi_async_locked\n");
		return ret;
	}
	
	atomic_inc(&cxt->pending);
	return ret;
}
...
static int max110x0i_setup(struct spi_context *spi_cxt) {
	uint8_t *tx_buf, *rx_buf; 
	int size = sizeof(struct adc_sample) * SAMPLES_PER_BUFFER; // spi msg size

	atomic_set(&(spi_cxt->pending), 0);
	...
	spi_cxt->xfer.rx_buf = rx_buf;
	spi_cxt->xfer.tx_buf = tx_buf;
	spi_cxt->xfer.len = size;

	spi_message_init(&spi_cxt->msg);
	spi_cxt->msg.complete = max110x0i_complete;
	spi_cxt->msg.context = spi_cxt;
	/* read data register command */
	//tx_buf[0] = MAX110X0_REG_RD_DATA;
	spi_message_add_tail(&(spi_cxt->xfer), &(spi_cxt->msg));
	return 0;
}
...
static int max110x0i_spi_probe(struct spi_device *spi)
{
	...
	return max110x0i_setup(spi_cxt);	
}
...
static void __exit max110x0i_exit(void)
{
	...
	atomic_t *p = &(max110x0i_data->cxt->pending);

	/* deregister data_cxt handler */
	free_irq(MAX110X0_IRQ, max110x0i_data->cxt);

	while(atomic_read(p)) /* wait the end of the last spi xfer */
		msleep(10);
