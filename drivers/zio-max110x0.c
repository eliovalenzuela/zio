...
struct adc_sample {
	...
	uint8_t adc_sequence[2]; /* generated by adc */
	uint8_t data[64][3];
};
...
static void max110x0i_complete(void *arg) {
	...
	if ((uint16_t) (ibuf[0].sequence - last_seqn) != 10)
...
static irqreturn_t max110x0i_gpio_irq(int irq, void *arg)
{
	...
	if (gpio_get_value(110)) {
		// FIXME: spurious irq
		//printk("spurious irq\n");
		return ret;
	}
	
	if (spi_async_locked(cxt->spi, &(cxt->msg))) {
